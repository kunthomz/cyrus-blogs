...
function get_css_font($option) {
	$font = OrchardOptions::get($option);
	if (!empty($font)) {
	...
	$font_family = OrchardOptions::get($option . '__custom_family');
	if ($font_family) {
...
function get_css_border($option, $top = true, $right = true, $bottom = true, $left = true) {
	$border = OrchardOptions::get($option);
	$border_color = OrchardOptions::get($option . '_color');
	if (!empty($border)) {
...
function get_css_padding($option) {
	$padding = OrchardOptions::get($option);
	if (
...
function get_css_color($option) {
	$color = OrchardOptions::get($option);
	if (!empty($color)) {
		echo 'color:' . $color . ';';
	}
}
...
$font = OrchardOptions::get('general_styles--font_second');
if (
...
$font_family = OrchardOptions::get('general_styles--font_second__custom_family');
...
$accent_color = esc_attr(OrchardOptions::get('general_styles--accent'));
...
<?php
$boxed = OrchardOptions::get('layout--boxed');
...
<?php
	...
	$border = OrchardOptions::get('layout--border');
	if (!empty($border)) {
?>
...
if (OrchardOptions::get('layout--header_width') == 'compact') {
	?>
...
elseif (OrchardOptions::get('layout--header_width') == 'expanded') {
	?>
...
if (OrchardOptions::get('layout--content_width') == 'compact') {
	?>
...
elseif (OrchardOptions::get('layout--content_width') == 'expanded') {
	?>
	@media (min-width: 768px) {
		.main-cnts-w > .container,
		.main-cnts-before > .container,
		...
		<?php if (get_post_type() == 'post' && OrchardOptions::get('posts--view') == 'metro') { ?>
...
if (OrchardOptions::get('layout--footer_width') == 'compact') {
	?>
...
elseif (OrchardOptions::get('layout--footer_width') == 'expanded') {
	?>
	@media (min-width: 768px) {
		.main-f-top > .container,
		.main-f-bottom > .container {
...
if (OrchardOptions::get('top_header')) {
?>
...
?>
	...
	.main-h-bottom:not(.__fixed) .mods-w.__with_separator:before {
		<?php
		$border_color = OrchardOptions::get('header_styles--border_color');
		if (!empty($border_color['rgba'])) {
			echo 'border-color:' . $border_color['rgba'] . ';';
		}
		?>
...
?>
	...
	.t-w {
		...
		echo 'text-align:' . OrchardOptions::get('title_wrapper_styles--align') . ';';
		?>
	}
	...
	.t-w_subcat {
		<?php
		$sub_header_color = OrchardOptions::get('title_wrapper_styles--font_title');
	...
	@media (min-width: 768px) {
		.t-w_desc {
			<?php
			echo 'margin-' . OrchardOptions::get('title_wrapper_styles--align') . ':0;';
			?>
		}
	}
...
if (OrchardOptions::get('bottom_footer')) {
?>
